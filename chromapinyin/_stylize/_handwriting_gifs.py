import os
import time
from ._res_directories import get_handwriting_dir
...
def process_gifs(
	fps, start_freeze_ms=1500, end_freeze_ms=3500, loops=True
):
	...
	handwriting_dir = os.path.join(get_handwriting_dir(), "images")
	large_handwriting_dir = os.path.join(get_handwriting_dir(), "images-large")
	...
	handwriting_dir = os.path.join(get_handwriting_dir(), "images-large")
	if not os.path.exists(handwriting_dir):
		print(f"{handwriting_dir} could not be found.")
	else:
		file_names = os.listdir(handwriting_dir)
		...
		for i, file_name in enumerate(file_names):
			...
			new_gif_path = os.path.join(
				handwriting_dir, file_name.replace("-large", "")
			)
...
def _modify_gif(
	gif_path, frame_delay_ms, start_freeze_ms, end_freeze_ms, loops=True
):
	try:
