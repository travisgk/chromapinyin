...
def create_word_list(hanzi_str, pinyin_str):
    ...
    while i < len(flat_inflections) and h < len(hanzi_list):
        if hanzi_list[h] not in PUNCTUATION and flat_inflections[i] in VOICELESS:
        ...
        if flat_inflections[i] == TO_INFLECTION["neutral"]:
            ...
            elif hanzi_list[h] == "不" and next_inflection == TO_INFLECTION["falling"]:
    ...
    apply_sequential_rule(
        inflections, TO_INFLECTION["low"], TO_INFLECTION["rising_low"]
    )

    # applies rule to make falling tones cut short.
    apply_sequential_rule(
        inflections, TO_INFLECTION["falling"], TO_INFLECTION["half_falling"]
    )
    ...
    for i in range(len(inflections)):
        ...
        if inflections[i][-1] == TO_INFLECTION["low"] and (
            i + 1 >= len(inflections)
            or (
                inflections[i + 1][0]
                in [
...
def _word_unit_to_syllables(results, unit_str):
    ...
    while len(unit_str) > 0:
        ...
        for i, char in enumerate(unit_str):
            ...
            elif not is_pinyin_vowel(char) and any(
                is_pinyin_vowel(letter) for letter in unit_str[:i]
            ):
