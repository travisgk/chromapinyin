...
from ._formatting_helper import reset_tabulation, embed_styling, HTML_line, colspan_str
...
def create_stylized_sentence(
    ...
    for word_i, word in enumerate(word_list):
        ...
        next_word = word_list[word_i + 1] if word_i + 1 < len(word_list) else None
        next_word_is_clause_break = False
        if next_word and next_word[0]["hanzi"] in CLAUSE_BREAKS:
            n_new_syllables += 0 if hide_clause_breaks else len(word_list[word_i + 1])
        ...
        [
            sum(
                [
                    len(word)
                    for word in line
                    if not hide_clause_breaks or word[0]["hanzi"] not in CLAUSE_BREAKS
                ]
            )
    ...
    # builds the HTML.
    result = HTML_line(f"<table {embed_styling([CHROMA_TABLE], use_css)}>", 1)
...
def _return_syllable_row_HTML(
    ...
    for categories_row in categories_2D:
        result += HTML_line(f"<tr {embed_styling([CHROMA_TR], use_css)}>", 1)
...
#
def _return_additional_punctuation(syllable_row, category, syllable_i, use_css):
...
def _return_syllable_td_HTML(
    ...
    if use_colspan:
        for i in range(category_i + 1, syllable_cells_width):
            if i >= len(categories_row) or (
                i < len(categories_row) and categories_row[i] == None
            ):
                colspan += 1
    ...
    if category_name == "hanzi":
        result += return_hanzi_contents(syllable, category, use_css, colspan, vertical)
