...
def set_font_sizes(scale=1.0):
    ...
    set_pitch_graph_height(
        f"{(_DEFAULT_PITCH_GRAPH_HEIGHT * scale):.3f}{_DEFAULT_UNIT}"
    )
    set_handwriting_height(
        f"{(_DEFAULT_HANDWRITING_HEIGHT * scale):.3f}{_DEFAULT_UNIT}"
    )
...
def set_pinyin_font_size(font_size=f"{_DEFAULT_PINYIN_FONT_SIZE:.3f}{_DEFAULT_UNIT}"):
    ...
    _CONTENT_STYLES["CHROMA_TD_PINYIN"] = {
        ...
        "style": ("margin: 0;", "padding: 0;", f"font-size: {font_size};"),
    }
    _TO_TD_STYLE["pinyin"] = _CONTENT_STYLES["CHROMA_TD_PINYIN"]


def set_zhuyin_font_size(
    font_size=f"{_DEFAULT_ZHUYIN_ROOT_FONT_SIZE:.3f}{_DEFAULT_UNIT}",
):
