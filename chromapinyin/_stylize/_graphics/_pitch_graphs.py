# chromapinyin._stylize._graphis._pitch_graphs.py
# ---
...
def _load_templates(style_name):
    ...
    _thinned_templates = {
        template_inflection: _load_thinned_graph(
            os.path.join(
                style_dir, f"_{find_inflection_label(template_inflection)}.png"
            )
        )
...
def _find_first_opaque_column(image, going_right):
    ...
    result_x = _column_search(image, 0 if going_right else width - 1, span_x)
    if not going_right:
        result_x += 1
    return result_x
